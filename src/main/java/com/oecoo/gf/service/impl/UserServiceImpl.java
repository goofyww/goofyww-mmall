...
import com.oecoo.gf.common.TokenCache;
import com.oecoo.gf.dao.UserMapper;
import com.oecoo.gf.pojo.User;
import com.oecoo.gf.service.IUserService;
import com.oecoo.gf.util.MD5Util;
...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> register(User user) {
        ...
        //代码复用 注册之前先校验
        ServerResponse validResponse = this.checkValid(user.getUsername(),Const.UserName);//检查用户名是否有重复
        if(!validResponse.isSuccess()){
    ...
    public ServerResponse<String> selectQuestion(String username){
        ServerResponse validResponse = checkValid(username,Const.UserName);//校验
        // 不存在 则为 true
        if(validResponse.isSuccess()){
            return ServerResponse.createByErrorMessage("用户名不存在");
        }
    ...
    public ServerResponse<String> checkAnswer(String username,String question,String answer){
        ...
        if(StringUtils.isBlank(answer)){
            return ServerResponse.createByErrorMessage("并没有接收到提交的答案，请重新提交");
        }
        //开始检查
        ...
        if(resultCount > 0){
            ...
            //TokenCache                 token_+username 防止横向越权 纵向越权 保证这个key是唯一的
    ...
    public ServerResponse<String> beforeLoginRestPassword(String username, String passwordNew, String forgetToken) {
        ...
        String token = TokenCache.getKey(TokenCache.TOKEN_PREFIX+username);//获取缓冲中的token
